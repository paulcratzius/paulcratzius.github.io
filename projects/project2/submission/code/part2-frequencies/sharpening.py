...
def unsharp_mask(img, ksize=9, sigma=1.6, alpha=1.0):
    """
    img in [0,1] (H,W[,C]), returns: blur, high (img - blur), sharp = clip(img + alpha*high)
    """
...
def run_unsharp_for_images(
    ...
    for name in images:
        ...
        # save original once (HTML nutzt *_orig.jpg)
        ...
        for sigma in sigma_list:
            for alpha in alpha_list:
                ...
                if high.ndim == 3:
                    high_vis = np.linalg.norm(high, axis=2)
                else:
                    high_vis = high
...
# ---------- EVALUATION: sharp -> blur (σ_b) -> re-sharpen (σ_s, α) ----------
def resharpen_after_artificial_blur(
    image_name="half-dome.jpg",
    out_dir="projects/project2/outputs/part2_unsharp",
    ksize=9,
    sigma_blur=2.0,         # wie im HTML erwartet (s2.00)
    sigma_unsharp=2.0,      # Schärfungssigma (gleich lassen ist ok)
    alphas=(0.5, 1.0, 1.5)  # HTML erwartet genau diese α
):
    os.makedirs(out_dir, exist_ok=True)

    # 1) Original laden/speichern
    in_path = f"projects/project2/images/{image_name}"
    ...
    # 2) Künstlich blur
    K_blur = gaussian_kernel(ksize, float(sigma_blur))
    blurred = conv_same(img01, K_blur)
    imwrite(f"{out_dir}/{stem}_blurred_for_eval.jpg", to_uint8(blurred))

    # 3) Re-sharpen auf dem BLURRED Bild (klassisches Unsharp auf dem Blur)
    for a in alphas:
        # Unsharp auf der verschwommenen Version (eigene zweite Glättung mit sigma_unsharp)
        _, _, re_sharp = unsharp_mask(blurred, ksize=ksize, sigma=float(sigma_unsharp), alpha=float(a))
        imwrite(f"{out_dir}/{stem}_resharp_a{a:.1f}.jpg", to_uint8(re_sharp))

if __name__ == "__main__":
    # Teil 1: normale Unsharp-Ergebnisse (blur/high/sharp) für alle Bilder
    ...
    # Teil 2: Evaluation – sharp -> blur -> re-sharpen (Dateinamen so, wie dein HTML sie lädt)
    resharpen_after_artificial_blur(
        image_name="half-dome.jpg",
        out_dir="projects/project2/outputs/part2_unsharp",
        ksize=9,
        sigma_blur=2.0,
