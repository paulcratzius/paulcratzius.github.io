...
def radial_mask(h, w, center=None, radius=0.35, feather=0.10, A_inside=True):
    """Circular (irregular-ish) feathered mask."""
    if center is None:
        center = (h*0.5, w*0.5)
    yy, xx = np.mgrid[0:h, 0:w].astype(np.float32)
    rr = np.sqrt((yy-center[0])**2 + (xx-center[1])**2)
    r0 = radius * min(h, w)
    t = (rr - r0) / (feather*min(h, w) + 1e-6)
    s = 1.0 / (1.0 + np.exp(t))  # inside≈1, outside≈0
    M = s[..., None] if A_inside else (1.0 - s)[..., None]
    return M

def polygon_mask(h, w, points, feather_px=25):
    """
    Irregular mask from polygon vertices (x,y). Inside=1, outside=0; feathered.
    """
...
def crop_match_with_anchor(A, B, anchor_y='center'):
    """
    Center-crop both images to the same size (min height/width).
    - Horizontally: always center.
    - Vertically: anchor_y in {'top','center','bottom'}.
      'top'   -> keep the top, trim from the bottom
      'center'-> symmetric crop (default old behavior)
      'bottom'-> keep the bottom, trim from the top
    """
    H = min(A.shape[0], B.shape[0])
    W = min(A.shape[1], B.shape[1])
...
def run_custom_blend(
    ...
    levels=5, sigma=2.0,
    mask_kind="vertical",           # 'vertical' | 'radial' | 'polygon' | 'triangle'
    mask_kwargs=None,
    ...
    elif mask_kind == "radial":
        M = radial_mask(H, W, **{
            "center": mask_kwargs.get("center", (H*0.55, W*0.5)),
            "radius": mask_kwargs.get("radius", 0.33),
            "feather": mask_kwargs.get("feather", 0.12),
            "A_inside": mask_kwargs.get("A_inside", True)
        })
    elif mask_kind == "polygon":
        pts = mask_kwargs.get("points", [(20,20),(W-20,50),(W-40,H-40),(30,H-60)])
        feather_px = mask_kwargs.get("feather_px", 35)
        M = polygon_mask(H, W, pts, feather_px)
...
if __name__ == "__main__":
    ...
    # --- Custom blend 2: Mountain ↔ Skyline (irregular polygon mask) ---
    # Polygon roughly around the mountain; tweak points if needed.
    # --- Custom blend 2: Mountain ↔ Skyline (triangle mask) ---
    # --- Custom blend 2: Mountain ↔ Skyline (centered crop + centered triangle mask) ---
    run_custom_blend(
